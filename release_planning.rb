...
parser = OptionParser.new do |opts|
  ...
  opts.on('-g', '--tag-regex REGEX', 'Tag regex') { |v| options[:tag] = v }
...
repos.each do |repo|
  begin
    latest_tag = util.fetch_tags("#{options[:namespace]}/#{repo}", options[:tag]).first
...
due_for_release.each do |entry|
  puts "#{entry['repo']} is due for release. Last release was tagged on #{entry['date']} and there have been #{entry['commits']} since then."
end
